# 🚀 My Portfolio

Welcome to my portfolio repository! This serves as a showcase of my skills, projects, and experience in software development, actuarial science, and cybersecurity. 

## 🌍 About Me
I am a passionate developer with expertise in:
- **Full-Stack Development** (MERN stack, Node.js, MySQL, HTML, CSS, JavaScript)
- **Software Engineering & Quality Assurance**
- **Cybersecurity Enthusiast**
- **Actuarial Science & Risk Management**
- **Blockchain Integration in Insurance & Healthcare**
- **AI for Software Engineering**

## 📌 Current Projects
- **E-Commerce Platform**: A marketplace with GPS-based delivery integration.
- **Telemedicine App**: AI-powered medical consultation platform with blockchain security.
- **Portfolio for Hackathons**: A dynamic showcase of my work and achievements.

## 🔧 Tech Stack
- **Backend:** Node.js,
- **Frontend:** HTML, CSS, JavaScript (No frameworks for now)
- **Database:** MySQL
- **DevOps & Cloud:** Azure
- **Version Control:** Git & GitHub

## 🎯 Goals
- Complete the 15-week PLP Scholarship ✅
- Deep dive into cybersecurity and quality assurance 🚀
- Land a job before returning to complete my degree 🎯
- Transition into cybersecurity 🔐

## 📬 Contact Me
- **GitHub:** catchmeinreal-life
- **Portfolio Website:** 
- **Email:** ericmatutu125@gmail.com

Feel free to explore my projects and reach out if you have any opportunities or collaborations in mind! 🚀
